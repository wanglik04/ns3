...
namespace ns3 {
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		{ //This is not QCN feedback, not NACK, or I am a switch so I don't care
			if (ipv4h.GetProtocol() != 0xFE) //not PFC
			{
					...
					if (ipv4h.GetProtocol() == 17)	//look at udp only
					{
						...
						for (i = 0; i < m_ecn_source->size(); ++i)
						{
							if ((*m_ecn_source)[i].source == ipv4h.GetSource() && (*m_ecn_source)[i].qIndex == GetPriority(p->Copy()) && (*m_ecn_source)[i].port == udph.GetSourcePort())
							{
								...
								//std::cout << (*m_ecn_source)[i].qfb << " is the qfb\n";
						...
						//if (thefirst) {
						//	found = true;
						//	if (pkt_cnt++ > 10000) thefirst = false;
						//}
						if (!found)
						{
							ECNAccount tmp;
						...
						int x = ReceiverCheckSeq(sth.GetSeq(), key); // 5 for usual
						...
						if (x == 2) //generate NACK
						{
							...
							//seqh.SetPG(GetPriority(p->Copy()));
							//seqh.SetPort(udph.GetSourcePort());
							//std::cout << "UDP_buffer_Nack_qbbHeader: " << GetUsedBuffer(seqh.GetPort(), seqh.GetPG()) << "\n\n";
							newp->AddHeader(seqh);
						...
						else if (x == 1) //generate ACK
						{
							...
							//seqh.SetPG(GetPriority(p->Copy()));
							//seqh.SetPort(udph.GetSourcePort());
							//std::cout << "UDP_buffer_ack_qbbHeader: " << GetUsedBuffer(seqh.GetPort(), seqh.GetPG()) << "\n\n";
							newp->AddHeader(seqh);
							Ipv4Header head;	// Prepare IPv4 header
							head.SetDestination(ipv4h.GetSource());
							Ipv4Address myAddr = m_node->GetObject<Ipv4>()->GetAddress(m_ifIndex, 0).GetLocal();
				...
				//log_file << Simulator::Now().GetSeconds() << " instant buffer: " << GetUsedBufferTotal() << std::endl;
				//log_file.close();
		...
		else if (ipv4h.GetProtocol() == 0xFF)
		{
			...
			//std::cout << "ECN_buffer: " << GetUsedBuffer(udpport, qIndex) << "\n\n";
			uint32_t i;
		...
		else if (ipv4h.GetProtocol() == 0xFD)//NACK on NIC
		{
			...
			//std::cout << "PFC_buffer_butNACK: " << GetUsedBuffer(port, qIndex) << "\n\n";
			int i;
			for (i = 1; i < m_queue->m_fcount; i++)
			{
			...
			else
			{
				...
				{
					std::cout << "ERROR: Sendingbuffer miss!2\n";
				}
				else printf("buffer len2: %u", &buffer_seq);
		...
		else if (ipv4h.GetProtocol() == 0xFC)//ACK on NIC
		{
			...
			int seq = qbbh.GetSeq();
			int port = qbbh.GetPort();
			//std::cout << "PFC_buffer: " << GetUsedBuffer(port, qIndex) << "\n\n";

			//if (thefirst) {
			//CheckandSendQCN(ipv4h.GetSource(), qIndex, port);
			//thefirst = false;
			//}
	...
	uint32_t
		QbbNetDevice::GetUsedBufferTotal() {
		return m_node->m_broadcom->GetUsedBufferTotal();
	}
